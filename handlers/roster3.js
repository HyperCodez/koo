...
module.exports = (client) => {
    ...
    client.on("guildMemberUpdate", function (oldMember, newMember) {
        try {
            const oldRoles = Array.from(oldMember.roles.cache.keys()).filter(x => !Array.from(newMember.roles.cache.keys()).includes(x))
            const newRoles = Array.from(newMember.roles.cache.keys()).filter(x => !Array.from(oldMember.roles.cache.keys()).includes(x))
