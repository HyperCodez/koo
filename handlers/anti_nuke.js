...
module.exports = client => {
                ...
                let AuditData = await member.guild.fetchAuditLogs({
                    ...
                    if (data.all.logger && data.all.logger.length > 5) {
                        try {
                            ...
                            console.log("ANTI-NUKE SYSTEM - ERROR-CATCHER".dim.cyan, e.stack ? String(e.stack).grey.italic.dim.grey : String(e).grey.italic.dim.grey)
                            return;
                        }
                    }
                    return;
            ...
            try {
                ...
                if (therole && therole.id) {
                    try {
                        ...
                        console.log(String(e.stack).grey.italic.dim.red);
                      }
                }
            } catch (e) {
                console.log("ANTI-NUKE SYSTEM - ERROR-CATCHER".dim.cyan, e.stack ? String(e.stack).grey.italic.dim.grey : String(e).grey.italic.dim.grey)
    ...
    client.on("roleCreate", async (role) => {
        try {
            if(!client.settings.has(role.guild.id, "language")) client.settings.ensure(role.guild.id, { language: "en" });
