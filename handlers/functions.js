...
async function edit_Roster_msg(client, guild, the_roster_db) {
  try{
    ...
    let message = await channel.messages.fetch(data.rostermessage).catch((e)=>{
      console.log("ERROR CATCHING, EDIT_MSG", e.stack ? String(e.stack).grey.italic.dim.bgRed : String(e).grey.italic.dim.bgRed)
    });
      ...
      try{
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              `<div class="message">`;
              if (msg.content) {
                subcontent += `<div class="content"><div class="markdown"><span class="preserve-whitespace">${markdowntohtml(String(msg.content).replace(/\n/ig, "<br/>"))}</div></div>` 
              } 
              if (msg.embeds[0]){
...
function stations(client, prefix, message) {
  ...
  try {
    ...
    stationsembed3.addField("🧾 OTHERS", `>>> ${requests}`, true);
    message.channel.send({embeds: [stationsembed]}).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
    message.channel.send({embeds: [stationsembed2]}).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
    message.channel.send({embeds: [stationsembed3]}).catch(e => console.log("THIS IS TO PREVENT A CRASH"))
  } catch (e) {
    console.log(String(e.stack).grey.italic.dim.bgRed)
  }
}
              ...
              try{
                  if(client.jointocreatemap.get(`tempvoicechannel_${vc.guild.id}_${vc.id}`) == vc.id){
                      ...
                      if(!members || members == undefined || members.length == undefined || members.length == 0){
                          ...
                          vc.delete().catch(e => console.log(e.stack ? String(e.stack).grey.italic.dim : String(e).grey.italic.dim) )
                      }
                  }
              }catch (e){
...
function create_join_to_create_Channel(client, user, type) {
  if(!client.settings.has(user.member.guild.id, "language")) client.settings.ensure(user.member.guild.id, { language: "en" });
