...
module.exports = function (client, options) {
  ...
  client.Jobmembercount = new CronJob('30 */10 * * * *', async function() {
    ...
    console.log(JSON.stringify(logguilds.map(guild => {if(guild && guild.name) return guild; else return "";}).filter(Boolean)).italic.yellow + " MEMBERCOUNTER ALL GUILDS")
  ...
  async function memberCount(guildid){
    ...
    if(!guild) return;
    //get all guilds
    await guild.members.fetch().catch(e => console.log(String(e).grey.italic.dim));
    //get the settings 
    let set = client.setups.get(guild.id, "membercount");
    //If no settings found, or defined on "no" return
    if(!set || !set.enabled) return
    if(set.channel1 && set.channel1.length == 18) Channel1();
    if(set.channel1 && set.channel1.length == 18) await delay(1000 * 60 * 2)
    if(set.channel2 && set.channel2.length == 18) Channel2();
    if(set.channel2 && set.channel2.length == 18) await delay(1000 * 60 * 2)
    if(set.channel3 && set.channel3.length == 18) Channel3();
