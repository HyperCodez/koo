...
invitedstring = `> Was invited by: **${ts}**`.substr(0, 1024);
  ...
  async function msg_withoutimg(member) {
    ...
    //define the leave embed
      ...
      if(client.settings.get(member.guild.id, "leave.invite")) leaveembed.addField("\u200b", invitedstring)
      //send the leave embed to there
      channel.send({embeds: [leaveembed]}).catch(e=>console.log("This catch prevents a crash"))
  }
  async function dm_msg_withoutimg(member) {    
    ...
    if(client.settings.get(member.guild.id, "leave.invitedm")) leaveembed.addField("\u200b", invitedstring)
    //send the leave embed to there
    member.user.send({embeds: [leaveembed]}).catch(e=>console.log("This catch prevents a crash"))
  }
  ...
  async function dm_msg_autoimg(member) {
      try {
        ...
        //define the leave embed
        //send the leave embed to there
        member.user.send({embeds: [leaveembed], files: [attachment]}).catch(e=>console.log("This catch prevents a crash"))
        //member roles add on leave every single role
      } catch {
      }
  }
  async function msg_autoimg(member) {
      try {
        ...
        //define the leave embed
        //send the leave embed to there
        channel.send({embeds: [leaveembed], files: [attachment]}).catch(e=>console.log("This catch prevents a crash"))
        //member roles add on leave every single role
