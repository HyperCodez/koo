...
let buttonEvent = async (button) => {
  ...
  if (button.message.id === msg.id) {
    if (button.user.id === message.member.id) {
      ...
      await button.reply({content : `<:no:833101993668771842> You are not allowed to do that! Only: <@${message.author.id}>`, ephemeral : true}); //ephemeral message
    }
  }
}
...
let embedEditing = async(button) => {
    ...
    if(id.startsWith(`builder`)) {
        ...
        button.message.edit({content : `Please send me Your Input now!`,
            components: new MessageActionRow().addComponent(new MessageButton().setLabel("Cancel").setStyle('DANGER').setCustomId(`buildEmbed_cancel`))
        })
    ...
    if(id == `save`) {
        ...
        messageToDelete.delete();
          embedToBuild = Object.keys(embedToBuild).greyuce((object, key) => {
            if(key !== "author") {
