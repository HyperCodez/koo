  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if ((Array.from(message.member.roles.cache.values()) && !message.member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(message.author.id) && (Array.from(message.member.roles.cache.values()) && !message.member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(message.guild.ownerID, config.ownerid).includes(message.author.id) && !message.member.permissions.has([Permissions.FLAGS_ADMINISTRATOR]))
        ...
        let Original_overrwites = message.guild.roles.cache.array().map(r => {
          let Obj = {}
          if(r.id){
            Obj.id = r.id;
            Obj.deny = ["SEND_MESSAGES", "ADD_REACTIONS"]
            Obj.allow = ["VIEW_CHANNEL"]
            return Obj;
          } else {
            return false;
          }
        }).filter(Boolean);
        let overrwites = Original_overrwites;
        if(Original_overrwites.length > 100) overrwites = overrwites.slice(0, 100);

        message.channel.overwritePermissions(overrwites, `Channel Lock by: ${message.author.tag} | ${message.author.id}`)
        .then(channel =>{
          message.react("🔓");
          try{
            for(const overwrite of overrwites.slice(100, overrwites.length - 1)){
              message.channel.createOverwrite(overwrite.id, {
                SEND_MESSAGES: false,
                VIEW_CHANNEL: true,
                ADD_REACTIONS: false
              })
        ...
        .catch(e=>{
          message.send(`${e.message ? e.message : e}`.substr(0, 1900), { code: "js"})
        });
