        ...
        try {
          ...
          if ((Array.from(message.member.roles.cache.values()) && !message.member.roles.cache.some(r => cmdroles.includes(r.id))) && !cmdroles.includes(message.author.id) && (Array.from(message.member.roles.cache.values()) && !message.member.roles.cache.some(r => adminroles.includes(r ? r.id : r))) && !Array(message.guild.ownerID, config.ownerid).includes(message.author.id) && !message.member.permissions.has([Permissions.FLAGS.ADMINISTRATOR]))
            ...
            const embed = new MessageEmbed()
              ...
              if(embed.fields[0]){
                embed.fields[0].name == '<:arrow:832598861813776394> __**Status:**__';
                embed.fields[0].value == `>>> ${String(statustext).substr(0, 1000)}`;
              } else {
                embed.addField('<:arrow:832598861813776394> __**Status:**__', `>>> ${String(statustext).substr(0, 1000)}`)
              }
              if(embed.fields[1]){
                embed.fields[1].name == '<:arrow:832598861813776394> __**Reason:**__';
                embed.fields[1].value == `>>> ${String(reason).substr(0, 1000)}`;
              } else {
                embed.addField('<:arrow:832598861813776394> __**Reason:**__', `>>> ${String(reason).substr(0, 1000)}`)
              }
              targetMessage.edit({embeds: [embed]})
              try{
                let usertag = targetMessage.embeds[0].author.name;
                await message.guild.members.fetch();
                let member = message.guild.members.cache.find(m=>m.user && m.user.tag && String(m.user.tag).toLowerCase() == String(usertag).toLowerCase())
