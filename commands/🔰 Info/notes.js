...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      var button_forward = new MessageButton().setStyle('blurple').setCustomId('notes_forwards').setEmoji('832598861813776394').setLabel("Forwards")
      var button_back = new MessageButton().setStyle('blurple').setCustomId('notes_backwards').setEmoji("833802907509719130").setLabel("Backwards")
      var button_jump = new MessageButton().setStyle('blurple').setCustomId('notes_jump').setLabel('Jump to Page').setEmoji("🔢");
      var button_empty1 = new MessageButton().setStyle('gray').setCustomId('notes_empty1').setLabel("\u200b").setDisabled(true)
      var button_list = new MessageButton().setStyle('blurple').setCustomId('notes_list').setLabel('List Notes').setEmoji("📑");
      
      var button_create = new MessageButton().setStyle('SUCCESS').setCustomId('notes_create').setEmoji('📋').setLabel("Create New Note")
      var button_edit = new MessageButton().setStyle('blurple').setCustomId('notes_edit').setEmoji('✏️').setLabel("Edit this Note")
    ...
    const collector = notemsg.createMessageComponentCollector({filter: (i) => i.isButton() && i.user && i.user.id == cmduser.id && i.message.author.id == client.user.id, time: 600e3 }); //collector for 5 seconds
        ...
        if(b.user.id !== message.author.id)
          ...
          if(b.customId == "notes_create"){
            if(notes.length >= 25) return message.reply("You've reached the Limit of Notes which is **25**").then(msg=>{
              ...
              var err = false;
              await mmmm.channel.awaitMessages(m=>m.author.id == cmduser.id, {max: 1, time: 180e3}).then(async collected => {
                ...
                var err = false;
                await mmmmm.channel.awaitMessages(m=>m.author.id == cmduser.id, {max: 1, time: 180e3}).then(async collected => {
          ...
          if(b.customId == "notes_backwards") {
              if (currentPage !== 0) {
                ...
                await b.deferUpdate();
                await notemsg.edit(`***Click on the __Buttons__ to swap the NOTES***`,{content: `***Click on the __Buttons__ to swap the NOTES***`,embeds: [embeds[currentPage]], components: allbuttons}).catch(e=>{console.log(String(e).grey)});
              } else {
          ...
          if(b.customId == "notes_list") {
              ...
              var buttonRow2 = new MessageActionRow().addComponents([button_create, button_edit, button_Delete, button_disable])
              allbuttons = [buttonRow1, buttonRow2]
              await notemsg.edit(`***Click on the __Buttons__ to swap the NOTES***`,{content: `***Click on the __Buttons__ to swap the NOTES***`,embeds: [embeds[currentPage]], components: allbuttons}).catch(e=>{console.log(String(e).grey)}).catch(e=>{console.log(String(e).grey)});
          } 
          //go forward
          ...
          if(b.customId == "notes_disable") {
            ...
            await notemsg.edit(`***NOTE BUTTONS DISABLED***`,{content: `***NOTE BUTTONS DISABLED***`,embeds: [embeds[currentPage]], components: allbuttons}).catch(e=>{console.log(String(e).grey)});
          } 
          //Number Jump
