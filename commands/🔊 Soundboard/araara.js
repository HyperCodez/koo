...
module.exports = {
	...
	run: async (client, message, args, cmduser, text, prefix) => {
		...
		if (!client.settings.get(message.guild.id, "SOUNDBOARD")) {return message.reply({embeds: [new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(client.la[ls].common.disabled.title).setDescription(require(`../../handlers/functions`).handlemsg(client.la[ls].common.disabled.description, {prefix: prefix}))]});}
		const { channel } = message.member.voice;
		const botchannel = message.guild.me.voice.channel;
		if (!channel) {return message.reply({embeds: [new MessageEmbed().setTitle(eval(client.la[ls]["cmds"]["uwu"][`${CmdName}`]["variable1"])).setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)]});}
		if (botchannel) {return message.reply({embeds: [new MessageEmbed().setTitle(eval(client.la[ls]["cmds"]["uwu"][`${CmdName}`]["variable2"])).setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)]});}
