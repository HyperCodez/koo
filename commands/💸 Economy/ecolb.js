    ...
    try {
      ...
      ensure_economy_user(client, message.guild.id, user.id)
        ...
        collector.on("collect", async (reaction, user) => {
          try {
            if (reaction.emoji.name === reactionemojis[1] || reaction.emoji.customId === reactionemojis[1]) {
            ...
            } else if (reaction.emoji.name === reactionemojis[0] || reaction.emoji.customId === reactionemojis[0]) {
              if (currentPage !== 0) {
                --currentPage;
                queueEmbed.edit({ embeds: [embeds[currentPage].setFooter(`You are: Rank #${yourrank} ・ Page: ${currentPage + 1}/${embeds.length}`, user.displayAvatarURL({dynamic: true}))]});
              } else {
