...
`)
    ...
    function ask_emoji() {
      ...
      var cancel = false;
      message.reply(rermbed).then(msg => {
        msg.awaitReactions((reaction, user) => user.id == message.author.id, {
          ...
          if (collected.first().emoji.customId && collected.first().emoji.customId.length > 2) {
            msg.delete();
            object2.Emoji = collected.first().emoji.customId;
            if (collected.first().emoji.animated)
              object2.Emojimsg = "<" + "a:" + collected.first().emoji.name + ":" + collected.first().emoji.customId + ">";
            else
              object2.Emojimsg = "<" + ":" + collected.first().emoji.name + ":" + collected.first().emoji.customId + ">";
            return ask_role();
            ...
            message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable5"]));
            return finished();
          }
        }).catch(() => {
          if (!cancel) {
            message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable6"]));
            return finished();
          }
        });
        msg.channel.awaitMessages(m => m.author.id === message.author.id, {
          ...
          if (!cancel) {
            message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable7"]));
            return finished();
          }
        });
      })
      ...
      function ask_role() {
        ...
        message.reply(rermbed).then(msg => {
          msg.channel.awaitMessages(m => m.author.id == message.author.id, {
            ...
            if (!role) message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable9"]))
            if (role) {
              ...
              message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable10"]));
              return finished();
            }
          }).catch((e) => {
            console.log(e.stack ? String(e.stack).grey : String(e).grey)
            message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable11"]));
            return finished();
          });
        })
      }
        ...
        msg.awaitReactions((reaction, user) => user.id === message.author.id && emojis2.includes(reaction.emoji.name), {
          ...
          switch (collected.first().emoji.name) {
            ...
            case "2️⃣":
              ...
              break;
            default:
              message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable14"]))
              break;
          }
                ...
                msg.channel.awaitMessages(m => m.author.id === message.author.id, {
                  ...
                  if (collected.first().mentions.channels.filter(ch=>ch.guild.id==message.guild.id).first()) {
                    ...
                    channel.send(embed.setDescription(buffer)).then(msg => {
                      ...
                      message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable19"]))
                    })

                  } else {
                    message.reply(eval(client.la[ls]["cmds"]["setup"]["setup-reactionrole"]["variable20"]));
                    return;
                  }
