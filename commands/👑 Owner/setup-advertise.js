    ...
    try {
      ...
      async function first_layer(){
        ...
        let Selection = new MessageMenu()
          ...
          .setPlaceholder('Click me to setup the Advertising System!');  //message in the content placeholder
        ...
        .setColor(es.color)
        .setAuthor('Advertising Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6ab.png',  'https://discord.gg/sngXqWK2eP')
        .setDescription(eval(client.la[ls]["cmds"]["owner"]["setup-advertise"]["variable4"]))
        let used1 = false;
        //send the menu msg
        let menumsg = await message.channel.send({embeds : [MenuEmbed], components :[Selection]})
        //function to handle the menuselection
        function menuselection(menu) {
          let menuoptiondata = menuoptions.find(v => v.value == menu.values[0])
          let menuoptionindex = menuoptions.findIndex(v => v.value == menu.values[0])
          if(menu.values[0] == "Cancel") return menu.reply.send(eval(client.la[ls]["cmds"]["owner"]["setup-advertise"]["variable5"]))
          menu.reply.defer();
          used1 = true;
        ...
        client.on('clickMenu', (menu) => {
          if (menu.message.id === menumsg.id) {
            if (menu.clicker.user.id === cmduser.id) {
              if(used1) return menu.reply.send({content : `<:no:833101993668771842> You already selected something, this Selection is now disabled!`}, {ephermal : true});
              menuselection(menu);
            }
            else menu.reply.send({content : `<:no:833101993668771842> You are not allowed to do that! Only: <@${cmduser.id}>`}, {ephermal : true});
          }
        });
      }
