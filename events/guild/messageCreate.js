...
module.exports = async (client, message) => {
  try {
    ...
    if (botchannel.toString() !== "") {
      ...
      if (!botchannel.includes(message.channel.id) && !message.member.permissions.has("ADMINISTRATOR")) {
        try {
          message.react("833101993668771842").catch(e => { console.log(String(e).grey.italic.dim); });
        } catch {}
        not_allowed = true;
        return message.reply({embeds: [new Discord.MessageEmbed()
          ...
          .setDescription(`${client.la[ls].common.botchat.description}\n> ${botchannel.map(c=>`<#${c}>`).join(", ")}`)]}
            ...
            setTimeout(()=>{
              try {
                msg.delete().catch(e => console.log(e.stack ? String(e.stack).grey.italic.dim : String(e).grey.italic.dim));
              } catch {}
            }, 5000)
        });
      }
    ...
    if (command && !customcmd) {
      ...
      return message.reply({embeds: [new Discord.MessageEmbed()
        ...
        .setDescription(`\`\`\`${e.message ? e.message : e.stack ? String(e.stack).grey.italic.dim.substr(0, 2000) : String(e).grey.italic.dim.substr(0, 2000)}\`\`\``)]
      }).then(async msg => {
        setTimeout(()=>{
          try {
            msg.delete().catch(e => console.log(e.stack ? String(e.stack).grey.italic.dim : String(e).grey.italic.dim));
  ...
  return message.reply({embeds: [new MessageEmbed()
    ...
    .setDescription(`\`\`\`${e.message ? e.message : String(e).grey.italic.dim.substr(0, 2000)}\`\`\``)]});
}
}
/**
 * @INFO
